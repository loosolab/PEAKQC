...
#Show versions:
print('rpy2 --> ', ad.__version__)
print('anndata2ri -->', anndata2ri.__version__)
print('anndata -->', ad.__version__)
print('scanpy -->', sc.__version__)
print('episcanpy -->', epi.__version__)

snapatac = importr('SnapATAC')

def make_rds(snap_path, sample=None):
    
    robjects.globalenv['path_snap'] = snap_path

    if sample == None:
        filename = (os.path.split(snap_path))[-1]
        basename = (filename.rstrip('.snap'))
    else:
        basename = sample

    robjects.globalenv['path_rds'] = "/home/rstudio/data/rds/"+ basename + ".rds"

    robjects.r('''
            # create a function `f`
            x.sp = createSnap(path_snap, sample="MB")
            x.sp = addPmatToSnap(x.sp)
            saveRDS(x.sp, file=path_rds);
...
def make_Anndata(input_data,
    ...
    for extra in extra_annot:
        ...
        if (extra in input_data.keys()) and (extra not in adata.obs.columns) and (len(input_data[extra][1]) == len(adata.obs_names.tolist())):
            adata.obs[extra] = input_data[extra][1]
    ...
    for n in ['tsne', 'umap']:
        if n in input_data.keys():
            ...
            x1 = input_data[n][1]
            x2 = adata.obs_names.tolist()
            if isinstance(input_data[n][1], robjects.vectors.Matrix):
                numpy_array = np.array(input_data[n][1])
                
                if numpy_array.shape == (len(adata.obs_names.tolist()), 2):
...
def convert_index(adata):
    """
    Convert the index of the adata.var into a standard format.
    """
...
if __name__=='__main__':
    ...
    # Create the parser
    my_parser = argparse.ArgumentParser(description='Convert Snap to Anndata')

    # Add the arguments
    my_parser.add_argument('Path',
                        metavar='path',
                        type=str,
                        help='the path to the snapfile'
                        )
    ...
    data_dir = (os.path.split((os.path.split(input_path))[0]))[0]
    print(data_dir)
    print(input_path)


    basename = make_rds(input_path)

    adata = snap2anndata(rds_file_name=(basename+'.rds'),
                    path='/home/rstudio/data/rds',
                    mtx_name='pmat',
                    feature_names='peak',
                    #feature_names='feature', #if single mtx inn the object with GRanges
                    jaccard=False,
                    jaccard_key='jmat',
                    save_all_rds=False,
                    verbose=True,
                    copy=True,
                    save=data_dir+'/anndata/'+basename+'.h5ad')
